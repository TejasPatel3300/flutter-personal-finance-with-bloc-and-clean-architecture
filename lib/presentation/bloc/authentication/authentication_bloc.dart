...
class AuthenticationBloc extends Bloc<AuthenticationEvent, AuthenticationState> {
  final AuthenticationUseCase authUseCase = AuthenticationUseCase(userRepository: UserRepositoryImpl());
  ...
  Future<void> _onLoginRequest(UserLoginRequested event, Emitter<AuthenticationState> emit) async {
    final either = await authUseCase.loginUser(email: event.email, password: event.password);
    either.fold(
      (failure) => emit(AuthenticationState.failure(failure.message)),
      (user) {
        SharedPreferencesHelper().setString('loggedInUser', jsonEncode(user));
        return emit(AuthenticationState.authenticated('${user.id ?? ''}'));
      },
    );
  }
